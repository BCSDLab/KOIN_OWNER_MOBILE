...
data class InsertStoreUiState(
    ...
    val categoryIds: List<Int>
        get() = listOf(1, selectedCategoryId)
}
...
class InsertStoreViewModel(
    ...
    private fun loadCategories() = intent {
        ...
        try {
            val categories = storeRepository.getStoreCategories()
    ...
    fun onNameChanged(value: String) = blockingIntent {
        reduce { state.copy(name = value, errorMessage = "") }
    }

    fun onAddressChanged(value: String) = blockingIntent {
        reduce { state.copy(address = value, errorMessage = "") }
    }
    ...
    fun onToggleCard() = blockingIntent {
        reduce { state.copy(isCardOk = !state.isCardOk) }
    }

    fun onToggleBank() = blockingIntent {
        reduce { state.copy(isBankOk = !state.isBankOk) }
    }

    fun onToggleDelivery() = blockingIntent {
        reduce { state.copy(isDeliveryOk = !state.isDeliveryOk) }
    }
    ...
    fun onDescriptionChanged(value: String) = blockingIntent {
        reduce { state.copy(description = value, errorMessage = "") }
    }
    ...
    fun addCoverImage(file: PlatformFile) = intent(registerIdling = false) {
    ...
    private fun submit() = intent {
        ...
        try {
            val imageUrls = state.coverImages.map { file ->
            ...
            storeRepository.registerStore(
                ...
                phone = state.phone,
                delivery = state.isDeliveryOk,
                deliveryPrice = state.deliveryPrice.toIntOrNull() ?: 0,
                payCard = state.isCardOk,
                payBank = state.isBankOk,
