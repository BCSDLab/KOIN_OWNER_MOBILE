...
class MenuEditorViewModel(
    ...
    fun init(storeId: String, menuId: String?) {
        intent(registerIdling = false) {
            reduce {
                state.copy(storeId = storeId, menuId = menuId, isEditMode = menuId != null)
            }
        }
    ...
    private fun loadMenuAndCategories(storeId: String, menuId: String?) {
        intent {
            ...
            try {
                ...
                if (menuId == null) {
                    reduce {
                        state.copy(isLoading = false, menuCategories = allCategories)
                    }
                    return@intent
                }
                ...
                val restoredCategoryIds = categories
                    .filter { c -> c.menus.any { it.id.toString() == menuId } }
                    .map { it.id }
                ...
                if (menu != null) {
                    reduce {
                        state.copy(
                            ...
                            optionPrices = if (menu.optionPrices.isEmpty() ||
                                (menu.optionPrices.size == 1 && menu.optionPrices.first().option.isBlank())
                            ) {
                                emptyList()
                            } else {
                                menu.optionPrices.map { op ->
                                    MenuOptionPriceDraft(
                                        option = op.option,
    ...
    fun onOptionNameChanged(index: Int, value: String) = blockingIntent {
        ...
        if (index in updated.indices) {
            updated[index] = updated[index].copy(option = value)
        }
    ...
    fun onPriceChanged(index: Int, value: String) = blockingIntent {
        val updated = state.optionPrices.toMutableList()
        if (index in updated.indices) {
            updated[index] = updated[index].copy(price = BusinessFormatters.digitsOnly(value))
        }
        reduce { state.copy(optionPrices = updated) }
    }
    ...
    fun addOptionPrice() {
        intent(registerIdling = false) {
            reduce {
                state.copy(
                    optionPrices = state.optionPrices + MenuOptionPriceDraft(),
                    singlePrice = ""
                )
            }
        }
    ...
    fun toggleHidden() {
        intent(registerIdling = false) {
            reduce { state.copy(isHidden = !state.isHidden) }
        }
    }
        ...
        intent {
            ...
            try {
                ...
                val domainOptionPrices = if (hasOptions) {
                    state.optionPrices.map {
                        MenuOptionPrice(option = it.option, price = it.price.toIntOrNull() ?: 0)
                    }
                } else {
                    emptyList()
                }
...
data class MenuOptionPriceDraft(
    val option: String = "",
    val price: String = ""
)
